...
def get_missing_intervals(df, col):
    ...
    for i in range(len(df)):
        if pd.isna(df[col].iloc[i]):
            if not in_interval:
                if i > 0:
                    start = df['Date_time'].iloc[i - 1]
                in_interval = True
        elif in_interval:
            end = df['Date_time'].iloc[i]
            if pd.notna(start) and pd.notna(end):
                missing_intervals.append((start, end))
            in_interval = False

    if in_interval and start is not None and pd.notna(df['Date_time'].iloc[-1]):
        missing_intervals.append((start, df['Date_time'].iloc[-1]))
